...
# q_map = {}
q_map = pd.Series()
q_map = pd.read_csv('result3.csv', header=None)
...
for c in range(400000):
    ...
    while 0 in set(stopflags):
        ...
        # if epsilon < 0.2:
        #     for i in range(4):
        #         if 0 + i*0.04 <= epsilon < 0.04 + i*0.04:
        #             act = i + 1
        ...
        if epsilon < 0.1:
            for i in range(4):
                if 0 + i*0.02 <= epsilon < 0.02 + i*0.02:
                    act = i + 1
        else:
            l2 = [0, 0, 0, 0]
            l3 = [0, 0, 0, 0]
            ...
            act_list = [i for i, v in enumerate(
                q_s_a_ts) if v == max(q_s_a_ts)]
            random.shuffle(act_list)

            act = act_list[0]

        act_stopflags = number_to_stopflag(act)
        ...
        act = stopflag_to_number(act_stopflags)
        ...
        for i in range(4):
            ...
            if stopflags[i] == 0:
                l4[i] = len(reach(positions[i], 4))
        sorted_s = positions_sort(stopflags, l2, l3, l4, act)
        s_a_t = board_to_str(sorted_s[0], sum(
            points), sorted_s[1], sorted_s[2], sorted_s[3], number_to_stopflag(sorted_s[4]))
        ...
        for i in range(5):
            sorted_s = positions_sort(stopflags, l2, l3, l4, i)
            a = board_to_str(sorted_s[0], sum(points), sorted_s[1], sorted_s[2],
                             sorted_s[3], number_to_stopflag(sorted_s[4]))
            if not (a in q_map):
                q_map[a] = 0
            q_s_t2s.append(q_map[a])

        q_s_t2 = max(q_s_t2s)

        q_map[s_a_t] = q_map[s_a_t] + \
            (0.1 * ((reward + 0.96*q_s_t2) - q_map[s_a_t]))

    sys.stdout.write("\r%d /100000" % c)
    sys.stdout.flush()


q_map = pd.Series(q_map)


# with open('C:\\Users\\Shibaike Ryoya\\workspace\\anti_bingo\\Q-learning\\result2.txt', 'a', encoding='utf-8') as f:
#     f.write(str(q_map))

q_map.to_csv('result3-2(300).csv', header=False)
